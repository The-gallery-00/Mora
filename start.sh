...
echo "[3/6] Starting Python OCR (port 8000)..."
cd "$(dirname "$0")/ocr" && python3 app.py &
cd "$(dirname "$0")"

echo "[4/6] Starting Python LLM (port 8001)..."
cd "$(dirname "$0")/llm" && python3 app.py &
cd "$(dirname "$0")"

echo "[5/6] Starting Spring Boot (port 8080)..."
cd "$(dirname "$0")/backend" && ./mvnw spring-boot:run &
cd "$(dirname "$0")"
