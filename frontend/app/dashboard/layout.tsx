...
export default function DashboardLayout({
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (
        storageRef.current &&
        !storageRef.current.contains(e.target as Node)
      ) {
  ...
  function handleSearch(e: React.FormEvent) {
    ...
    if (searchQuery.trim()) {
      router.push(
        `/dashboard/search?q=${encodeURIComponent(searchQuery)}&type=${searchCategory}`,
      );
    }
  }

  function openStorageMenu() {
    const selectedIndex = STORAGE_ITEMS.findIndex(
  ...
  function moveActiveStorage(direction: 1 | -1) {
    setActiveStorageIndex(
      (prev) =>
        (prev + direction + STORAGE_ITEMS.length) % STORAGE_ITEMS.length,
    );
  }

  function handleStorageTriggerKeyDown(
  ...
  function handleStorageOptionKeyDown(
  ...
  function openCategoryMenu() {
    const selectedIndex = SEARCH_CATEGORIES.findIndex(
  ...
  function moveActiveCategory(direction: 1 | -1) {
    setActiveCategoryIndex(
      (prev) =>
        (prev + direction + SEARCH_CATEGORIES.length) %
  ...
  const currentCategory = SEARCH_CATEGORIES.find(
    (c) => c.value === searchCategory,
  );
    ...
    <div style={{ minWidth: 1200, minHeight: "100vh", background: "#FFFFFF" }}>
      ...
      >
        ...
        >
          ...
          <div
            ref={categoryRef}
            style={{ position: "relative", flexShrink: 0, width: 76 }}
          >
            <button
        ...
        >
          ...
          <Link
            ...
            style={{
              ...
              fontWeight: 500,
              textDecoration: "none",
              color: pathname.startsWith("/dashboard/upload")
                ? "#0077B6"
                : "#505050",
              background: pathname.startsWith("/dashboard/upload")
                ? "#F0F9FF"
                : "transparent",
          ...
          <div ref={storageRef} style={{ position: "relative" }}>
            <button
              ...
              style={{
                ...
                fontWeight: 500,
                cursor: "pointer",
                color: pathname.startsWith("/dashboard/storage")
                  ? "#0077B6"
                  : "#505050",
                background: pathname.startsWith("/dashboard/storage")
                  ? "#F0F9FF"
                  : "transparent",
                transition: "all 0.15s",
                outline: "none",
